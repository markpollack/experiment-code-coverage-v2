...
\midrule
\textbf{hardened+skills+pre-analysis} (run 17) & 69 & 11 & \$3.49 & 26 min \\
\textbf{hardened+skills+pre-analysis+plan-act} (run 14) & 97 & \textbf{23} & \textbf{\$5.67} & 43 min \\
\bottomrule
\end{tabular}
\end{center}
...
\midrule
\texttt{hardened+skills+pre-analysis}     & 3 &  75.0 & 8.7  & \$3.39 & 0.850 & 0.017 \\
\texttt{hardened+pre-analysis}            & 3 &  80.3 & 12.1 & \$3.41 & 0.789 & 0.035 \\
\texttt{hardened+kb}             & 3 &  83.3 & 7.4  & \$3.21 & 0.847 & 0.027 \\
\texttt{hardened+skills+pre-analysis+plan-act} & 3 & 95.0 & 11.1 & \$5.11 & 0.878 & 0.092 \\
...
\begin{warningbox}
\textbf{The plan-act paradox}: \texttt{hardened+skills+pre-analysis+plan-act} ranks 4th on efficiency
...
\texttt{hardened} $\to$ \texttt{hardened+pre-analysis}: $-$22\% steps,
...
\texttt{hardened+skills+pre-analysis}: $-$31\% steps vs simple, quality = 0.850 ---
...
\texttt{hardened+skills+pre-analysis} &  8.5\% & 17.9\% & pre-analysis reduces search overhead \\
\bottomrule
\end{tabular}
\end{center}
...
\subsection*{Exploration quality: not all orientation is equal}

EXPLORE (targeted file access via Read/Glob) is productive --- the agent \emph{should}
read source files before writing tests. SHELL (blind \texttt{find}, \texttt{ls}, \texttt{grep})
and JAR\_INSPECT (\texttt{jar tf}, \texttt{javap}) are wasteful.
Skills shift the agent from searching to reading:

\begin{center}
\begin{tabular}{@{}lrrrr@{}}
\toprule
\textbf{Variant} & \textbf{Read calls} & \textbf{SHELL calls} & \textbf{JAR calls} & \textbf{SEARCH \%} \\
\midrule
\texttt{simple}              & 100 & 33 & 20 & 24.3\% \\
\texttt{hardened}            &  89 & 62 & 54 & 37.9\% \\
\texttt{hardened+kb}         &  74 & 39 & 37 & 30.8\% \\
\texttt{hardened+preanalysis}          &  74 & 52 & 20 & 30.1\% \\
\texttt{hardened+skills}     & 173 & 35 &  5 & 13.2\% \\
\texttt{hardened+skills+preanalysis} &  87 & 21 & 19 & 17.9\% \\
\texttt{hardened+skills+preanalysis+plan-act} & 107 & 39 & 11 & 17.7\% \\
\bottomrule
\end{tabular}
\end{center}

Targeted Read calls nearly double from 89 (\texttt{hardened}) to 173 (\texttt{hardened+skills}),
while SHELL drops from 62 to 35 and JAR\_INSPECT plummets from 54 to 5.
Skills don't reduce how much the agent orients --- they shift it from searching to reading.
...
The heatmap for \texttt{hardened+skills+pre-analysis+plan-act} shows mutual high probability
between the BUILD and FIX rows: $P(\text{BUILD}\to\text{FIX})$ and
$P(\text{FIX}\to\text{BUILD})$ both elevated. Compare to \texttt{hardened+skills+pre-analysis}:
...
\texttt{hardened+pre-analysis}        &  80.3 &  80.3 & 13.3 \\
\texttt{hardened+skills}     & 101.7 & 101.7 & 11.0 \\
\texttt{hardened+skills+pre-analysis} &  75.0 &  75.0 & 10.3 \\
\bottomrule
\end{tabular}
\end{center}
\end{findingbox}
...
\begin{itemize}
  ...
  \item The FIX/BUILD spiral risk is variant-specific.
    plan-act has it; \texttt{hardened+skills+pre-analysis} does not.
\end{itemize}
...
\begin{enumerate}
  ...
  \item \texttt{hardened+skills+pre-analysis} --- skills + pre-analysis
  \item \texttt{hardened+skills+pre-analysis+plan-act} --- two-phase
\end{enumerate}
...
\begin{enumerate}[leftmargin=1.5em,itemsep=4pt]
  ...
  \item \textbf{Skills fix pre-analysis's quality regression.}
    \texttt{hardened+skills+pre-analysis}: $-$31\% steps, quality matches \texttt{hardened}.
