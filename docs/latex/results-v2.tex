...
\texttt{hardened+skills+sae+forge}
...
Every tool call was classified into one of 8 semantic states
(EXPLORE, READ\_KB, READ\_SKILL, JAR\_INSPECT, WRITE, BUILD, VERIFY, FIX).
...
\textbf{hardened+skills+sae+forge} (run 14) & 97 & \textbf{23} & \textbf{\$5.67} & 43 min \\
\bottomrule
\end{tabular}
\end{center}
...
\caption{Mean cost (left) and mean step count (right) per variant across all runs.
         The forge variant (red bar) has the highest mean cost and the highest variance ---
...
\texttt{hardened+skills+sae+forge} & 3 & 95.0 & 11.1 & \$5.11 & 0.878 & 0.092 \\
...
\begin{warningbox}
\textbf{The forge paradox}: \texttt{hardened+skills+sae+forge} ranks 4th on efficiency
...
\begin{tabular}{@{}lrc@{}}
\toprule
\textbf{Variant} & \textbf{JAR\_INSPECT \%} & \textbf{Intervention} \\
\midrule
\texttt{simple}            & 7.9\%  & none \\
\texttt{hardened}          & 11.0\% & prompt only (more thorough, no direction) \\
\texttt{hardened+kb}       & 10.1\% & flat KB helps but doesn't eliminate \\
\texttt{hardened+skills}   &  1.0\% & skills answer the import directly \\
\texttt{hardened+skills+sae} & 7.1\% & pre-analysis reduces JAR by rerouting (different mechanism) \\
\bottomrule
\end{tabular}
\end{center}
...
from 7.9\% to 11.0\%. The hardened prompt makes the agent more thorough about finding
...
The heatmap for \texttt{hardened+skills+sae+forge} shows mutual high probability
...
The forge variant's two-phase design (deep explore, then sustained act) produces
...
\begin{minipage}[t]{0.48\textwidth}
  ...
  \caption*{\textbf{Spiral variant (forge)}: BUILD$\to$FIX and FIX$\to$BUILD
...
\caption{The rework loop in the heatmap. Bright mutual probability between BUILD
         and FIX rows means the agent is cycling. The forge variant's two-phase structure
...
A formal second-order test found mean KL divergence of 0.49 bits across 27
(previous, current) state pairs --- first-order Markov is formally rejected ($p < 0.001$).
The violation is concentrated in recovery transitions:
FIX$\to$EXPLORE has 2.10 bits of second-order information.
...
\begin{itemize}
  ...
  \item The FIX/BUILD spiral risk is variant-specific.
    Forge has it; \texttt{hardened+skills+sae} does not.
\end{itemize}
...
\begin{enumerate}
  ...
  \item \texttt{hardened+skills+sae+forge} --- two-phase
\end{enumerate}
...
\begin{enumerate}[leftmargin=1.5em,itemsep=4pt]
  ...
  \item \textbf{Forge is high variance.}
